 ...
 */
package com.house.safetyandcheap.sms;
...
public class Smsc {
	 ...
	 * @param format - формат сообщения (0 - обычное sms, 1 - flash-sms, 2 - wap-push, 3 - hlr, 4 - bin, 5 - bin-hex, 6 - ping-sms, 7 - mms, 8 - mail, 9 - call, 10 - viber, 11 - soc, 12 - bot, 13 - telegram)
	 * @param sender - имя отправителя (Sender ID) или имя бота для telegram и whatsapp.
	...
	public String[] send_sms(String phones, String message, int translit, String time, String id, int format, String sender, String query)
	{
		...
		try {
			m = _smsc_send_cmd("send", "cost=3&phones=" + URLEncoder.encode(phones, SMSC_CHARSET)
					...
					+ (sender.isEmpty() ? "" : (format == 12 ? "&bot=" : "&sender=") + URLEncoder.encode(sender, SMSC_CHARSET))
	...
	private String[] _smsc_send_cmd(String cmd, String arg){
		...
		try {
			String _url = (SMSC_HTTPS ? "https" : "http") + "://smsc.ru/sys/" + cmd +".php?" + (SMSC_LOGIN != "" ? "login=" + URLEncoder.encode(SMSC_LOGIN, SMSC_CHARSET)
				+ "&psw=" : "apikey=") + URLEncoder.encode(SMSC_PASSWORD, SMSC_CHARSET)
